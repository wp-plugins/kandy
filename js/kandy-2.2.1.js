...
api.messaging = (function () {
    ...
    function _sendIM(destination, contentType, msg, success, failure, isGroup) {
        var url;
        var uuid = _UUIDv4();
        var message = {
            message: {
     ...
     */
    function _sendImWithAttachment(destination, attachment, contentType, success, failure, isGroup) {
        if (_config.messageProvider === 'fring') {
            ...
            me.uploadFile(attachment, function (fileUuid) {
                ...
                return _sendIM(destination, contentType, {
    ...
    me.sendImWithContact = function (user, vCard, success, failure) {
        return _sendImWithAttachment(user, vCard, _imContentTypes.CONTACT, success, failure);
    };
        ...
        xhr.onload = function () {
            if (xhr.status === 200) {
                ...
                if (JSON) {
                    ...
                    // TODO: Why are we evaluating the response of an XHR?
...
api.Phone = api.call = api.voice = (function () {
    ...
    /**
     * @property {Object} spidrAPIDefaults Default configuration for SPiDR api.
     * @private
     */
    var spidrAPIDefaults = {
        cors: true,
    ...
    /**
     * @property {Object} spidrMedia Defaults Default configuration for SPiDR api.
     * @private
     */
    var spidrMediaDefaults = {
        pluginMode: 'auto',
        pluginLogLevel: 2
    };
    ...
    function _mergeConfigWithSpidrConfiguration(spidrConfig) {
        ...
        _config.spidrApi = defaults(_mapSpidrConfigToAPI(spidrConfig), spidrAPIDefaults);

        // apply default SPiDR configuration
        _config.spidrMedia = defaults(_mapSpidrConfigToMedia(spidrConfig), spidrMediaDefaults);
    ...
    _logInToSpidr = function(success, failure) {
        ...
        _getSpidrConfiguration(_userDetails.user_access_token,
                function (spidrConfig) {
                    ...
                    var username = _userDetails.full_user_id;

                    fcs.setUserAuth(username, _userDetails.user_password);
                    ...
                    fcs.notification.start(
                            function () {
                                ...
                                if (_config.allowAutoLogin && _supportsLocalStorage()) {
                                    ...
                                    // TODO: password is not defined, use user_password?
    ...
    function _notificationHandler(message) {
        ...
        message = message.kandyType || message.message_type;
        if (message === 'gofetch') {
            _fireEvent('messagesavailable');
        } else if (message === 'incomingCall') {
            // TODO: Call id is not define, remove it?
            me._onIncommingCall('CALLavailable', call_id);
        }
    }
...
api.addressbook = (function () {
     ...
     */
    me.retrieveUserDeviceAddressBook = function (userAccessToken, success, failure) {
...
api.Registration = api.registration = (function () {
    ...
    me.getUserInfo = function (success, failure) {
        ...
        // TODO: _userAccessToken undefined ... doesn't make sense.
        var encodedAccessCode = encodeURIComponent(_userAccessToken);

        _kandyAjax({
            url: '/users/billing/packages/status/active?key=' + encodedAccessCode,
            success: function (response) {
                if (success) {
                    success(response.result);
                }
    ...
    me.getProfileInfo = function (userId, domaiId, success, failure) {
        var params = [
            // TODO: _userAccessToken undefined ... doesn't make sense.
            'key=' + encodeURIComponent(_userAccessToken),
